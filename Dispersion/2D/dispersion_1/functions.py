...
def load_excel_data(excel_path):  
  ...
  data_mass_frac = np.array(excel_data_frame['SPECIE_A']).reshape(data_size,1) 
  ...
  train_w =  data_mass_frac
  return [train_x, train_y , train_u, train_v, train_p, train_w]
...
def split_train_test(load_x, load_y , load_u, load_v, load_p, load_c, test_frac):
  # Input and output variables 
  var_input = np.concatenate((load_x, load_y),axis=1)
  var_output = np.concatenate((load_u, load_v, load_p,load_c),axis=1)
  # Split train test 
  ...
  train_u , train_v, train_p, train_c = np.split(var_out_train, 4, axis=1)  
  test_u , test_v, test_p, test_c = np.split(var_out_test, 4, axis=1)  
  return  train_x , train_y,train_u , train_v, train_p,train_c, test_x , test_y , test_u , test_v, test_p ,test_c 
...
def normalize_dataset_0_1(load_x,load_y,load_u,load_v,load_p,load_c): 
    ...
    return norm_load_x,norm_load_y,norm_load_u,norm_load_v,norm_load_p,norm_load_c
