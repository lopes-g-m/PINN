...
def generate_3D_vtk(test_x,test_y,test_z,test_c,title,savePath):    
    points = vtk.vtkPoints()
    for i in range(test_x.shape[0]):
        for j in range(test_x.shape[1]):
            points.InsertNextPoint(test_x[i, j], test_y[i, j], test_z[i, j])
        grid = vtk.vtkStructuredGrid()
    grid.SetDimensions(test_x.shape[0], test_x.shape[1], 1)
    ...
    for i in range(test_c.shape[0]):
        for j in range(test_c.shape[1]):
            scalar_array.InsertNextValue(test_c[i, j])    
    grid.GetPointData().AddArray(scalar_array)
    writer = vtk.vtkStructuredGridWriter()
    writer.SetFileName(str(savePath) + title + ".vtk")
